...
int main(int argc, char* argv[]) {
    ...
    //Задание 11
    sem_t* sem_read = sem_open("/posix_sem_read", O_CREAT, 0666, 3);
    if (sem_read == SEM_FAILED) {
        perror("Ошибка создания семафора");
        exit(EXIT_FAILURE);
    }

    sem_t* sem_write = sem_open("/posix_sem_write", O_CREAT, 0666, 1);
    ...
    switch (pid) {
        ...
        default:
            ...
            close(pipefd[0]);
            if (sem_unlink("/posix_sem_read") == -1) {
                perror("Ошибка удаления семафора");
                exit(EXIT_FAILURE);
            }
            if (sem_unlink("/posix_sem_write") == -1) {
                perror("Ошибка удаления семафора");
                exit(EXIT_FAILURE);
            }
            exit(EXIT_SUCCESS);
